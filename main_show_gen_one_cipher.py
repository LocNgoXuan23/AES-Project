...
def printMatrix(input):
  for i in range(4):
    for j in range(4):
      print('{:02x}'.format(input[i][j]), end=' ')
    print('\n')

def printMatrix2(input):
  for i in range(4):
    for arr in input:
      for j in range(4):
        if arr[i][j] == ' ':
...
def encryption(plaintext_txt, key_txt):
  ...
  count = 0
  print(f"Round {count}")
  print("Start of Rounds      After SubBytes       After ShiftRows      After MixColumns     Round Key")
  printMatrix2([plaintext, blank, blank, blank, round_key[:, 0:4]]) 
  ...
  for i in range(4, 40, 4):
    ...
    print("Start of Rounds      After SubBytes       After ShiftRows      After MixColumns     Round Key")
    endMatrix = [temp, after_sub, after_shift, after_mix, round_key[:, i:i+4]]
    printMatrix2(endMatrix)

  print(f"Start round {count + 1} (Final Round)")
  print("Start of Rounds      After SubBytes       After ShiftRows      After MixColumns     Round Key")
  ...
  printMatrix2([temp, after_sub, after_shift, blank, round_key[:, 40:44]])
...
def decryption(ciphertext_txt, key_txt):
  ...
  count = 0
  print(f"Round {count}")
  print("Start of Rounds      After ShiftRows      After SubBytes      After AddRoundKey     Round Key")
  printMatrix2([ciphertext, blank, blank, blank, round_key[:, 40:44]])
  ...
  for i in range(40, 4, -4):
    ...
    printMatrix2([after_mix, after_shift, after_sub, after_add, round_key[:, i-4:i]])
  ...
  printMatrix2([after_mix, after_shift, after_sub, after_add, round_key[:, 0:4]])
...
plaintext_txt = "0123456789abcdeffedcba9876543210"
key_txt = "0f1571c947d9e8590cb7add6af7f6798"

ciphertext_txt = encryption(plaintext_txt, key_txt)
print(ciphertext_txt)


print("========================================================================================")

plain = decryption(ciphertext_txt, key_txt)
print(plain)